        ...
        {features.map((feature, i) => (
          <li key={i} className="flex items-start gap-3">
            {feature.included ? (
              ...
              <svg className="w-5 h-5 text-[#5a5a70] flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
              </svg>
            )}
            {/* Not-included items: readable but clearly secondary. #353545 on
                #0a0a0f was ~1.4:1 contrast — invisible on phones. #8a8aa0 is
                ~4.5:1; the strikethrough + dim icon keep them visually inferior
                to the #e0e0e8 included rows. */}
            <span className={feature.included ? "text-[#e0e0e8] text-sm" : "text-[#8a8aa0] text-sm line-through decoration-[#8a8aa0]/70"}>
              {feature.text}
            </span>
          </li>
        ))}
